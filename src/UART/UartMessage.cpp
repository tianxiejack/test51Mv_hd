...
#define UART_BIT_SET(val,bit) (val|(1<<bit))
#define UART_BIT_CLR(val,bit) (val&(~(1<<bit)))
  ...
  switch (pData[3])
    ...
    case AckPowerOn:
        //OSA_printf("unit:%02x\n",pIStuts.unitFaultStat);
          if(FlirStat==2)
          	{
			pIStuts.unitFaultStat=UART_BIT_CLR(pIStuts.unitFaultStat,1);
    ...
    case AckTrkErr:
        //printf("errx:%d*****erry:%d\n",pIStuts.trkerrx,pIStuts.trkerry);
...
//	sendLen=EXTCTRL_sndTrkErrtest(pIStuts.trkerrx,pIStuts.trkerry,pData,pIStuts.SensorStat);
    ...
    case AckProId:
        ...
        sendLen = EXTCTRL_sndProductID(0x0104,18, 2, 25, pData);
        break;
    ...
    case AckTvCollErr:
	//OSA_printf("TempTvCollXY(%d,%d)\n",pIStuts.TempTvCollX,pIStuts.TempTvCollY);
...
int Uart_lookupSync(unsigned char *RS422,int *len)
{


    int stat = -1;
    Uint8 *pCur =RS422;
    extCommHead_t *pMsg = (extCommHead_t *)pCur;
    int *recvlen=len;

    while (*recvlen>= 3)
    {
        if (pMsg->SFCC == 0xCC && (pMsg->PckLen >= 5 && pMsg->PckLen <= MSG_PACKET_SIZE))
        {
            stat = 0;
            break;
        }
        else
        {
            pCur++;
            (*recvlen)--;
            pMsg= (extCommHead_t *)pCur;
        }
    }

    if(pCur!=RS422)
    {
        memcpy(RS422, pCur, (*recvlen));
    }
    return stat;
}
//
			 ...
			 case TarCoordinate: //for target xy 
		        ...
		        case SysInfoDisp:
					            ...
					            if(pMsg->SysMsgId == 0 || pMsg->SysMsgId == 1 ||pMsg->SysMsgId == 2)
					            {
					                ...
					                if((pMsg->SysMsgId == 0)||(pMsg->SysMsgId == 1))
					                {
